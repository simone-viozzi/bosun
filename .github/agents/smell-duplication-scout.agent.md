...
<focus>
Find and document duplication: copy/paste logic, near-duplicates, repeated boilerplate that should be centralized, and multiple inconsistent versions of the same business rule.
</focus>

<heuristics>
- Look for repeated blocks in the diff and nearby files.
- Search for similar function names, comments, error strings, and magic constants that repeat.
- Identify “same shape, different fields” code: similar control flow with small variations.
- Identify repeated validation/serialization/mapping logic across layers.
- Identify repeated orchestration sequences (“do A then B then C” repeated in many places).
</heuristics>

<evidence_checklist>
For each duplication smell, capture:
- Primary location (canonical candidate)
- Other occurrences (at least 2 if possible)
- What is truly common vs what varies
- Risk: inconsistent fixes, drift, test duplication, bug reintroduction
- Candidate extraction boundary:
  - helper function / shared module
  - strategy/policy object
  - data-driven configuration
</evidence_checklist>

<false_positives_to_avoid>
- Legitimate repetition mandated by framework or interface contracts.
- Small repetition that is more readable than abstraction (note it, but don’t over-push).
- Generated code or schema bindings (if present).
</false_positives_to_avoid>

<question_templates>
- “Is there an existing shared utility/module that should own this logic?”
- “Do these variants need to diverge intentionally, or should they be unified?”
- “Is it acceptable to introduce a shared abstraction in this layer, or must it stay local?”
</question_templates>

<stop_conditions>
- Stop when you can point to clear clusters of duplication with a plausible shared boundary.
- If scope is diff-only, prefer duplication introduced/modified in the diff plus immediate neighbors.
</stop_conditions>

<handoff_to_orchestrator>
If you see multiple duplications that are actually one root abstraction gap, say so explicitly and suggest merging under one smell entry.
</handoff_to_orchestrator>
</scout_specific_instructions>
