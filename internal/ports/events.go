...
type EventEmitter interface {
	// EmitJobScheduled is called when a job is registered with the scheduler.
	...
	// EmitJobCompleted is called when a job finishes successfully.
	...
	// EmitJobSkipped is called when a scheduled run is skipped (overlap policy).
	EmitJobSkipped(ctx context.Context, jobName string, reason string)

	// EmitJobAdded is called when config refresh discovers a new job.
	EmitJobAdded(ctx context.Context, jobName string)

	// EmitJobRemoved is called when config refresh detects a removed job.
	EmitJobRemoved(ctx context.Context, jobName string)

	// EmitJobChanged is called when a job's schedule or policy changes.
	...
	// EmitJobDuplicateName is called when duplicate job names are detected during discovery.
